...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        i=0
        place_throw=self.place
        while i<self.min_range:
            if place_throw.name=='Hive':
                return None
            place_throw=place_throw.entrance
            i+=1
        while i<=self.max_range:
            if place_throw.name !='Hive':
                if place_throw.bees != []:
                    return random_or_none(place_throw.bees)
                place_throw=place_throw.entrance
                i+=1
            else:
                return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            ...
            self.death_callback()
            for x in Bees:
                x.reduce_armor(amount+self.damage)
        else:
            for x in Bees:
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        QueenAnt.number_of_QueenAnt+=1
        self.number_of_QueenAnt=QueenAnt.number_of_QueenAnt
        # END Problem 13
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if self.number_of_QueenAnt==1:
            ThrowerAnt.action(self,gamestate)
            place_behind=self.place.exit
            while place_behind != None:
                if isinstance(place_behind.ant,ContainerAnt):
                    if hasattr(place_behind.ant,'damage') and place_behind.ant.mark==0:
                        place_behind.ant.damage *=2
                        place_behind.ant.mark +=1
                    if hasattr(place_behind.ant.contained_ant,'damage') and place_behind.ant.contained_ant.mark==0:
                        place_behind.ant.contained_ant.damage *=2
                        place_behind.ant.contained_ant.mark +=1
                else:
                    if hasattr(place_behind.ant,'damage') and place_behind.ant.mark==0:
                        place_behind.ant.damage *=2
                        place_behind.ant.mark +=1
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        make_slow_index=[i for i in range(len(self.effect)) if self.effect[i] is make_slow]
        make_scare_index=[i for i in range(len(self.effect)) if self.effect[i] is make_scare]
        make_slow_duration,make_scare_duration=[],[]
        for i in make_slow_index:
            make_slow_duration +=[self.effect_duration[i]]
        for i in make_scare_index:
            make_scare_duration +=[self.effect_duration[i]]

        if len(self.effect) !=0 and any(self.effect_duration):
            if gamestate.time %2 !=0:
                if make_slow in self.effect and any(make_slow_duration):
                    i=0
                    while make_slow_duration[i] ==0:
                        i+=1
                    index=make_slow_index[i]
                    self.effect[index](self.action,self)(gamestate)
                    self.effect_duration[index] -=1
                    if make_scare in self.effect and any(make_scare_duration):
                        j=0
                        while make_scare_duration[j] ==0:
                            j+=1
                        index2=make_scare_index[j]
                        if index2>index:
                            self.effect_duration[index2] -=1
                else:
                    i=0
                    while make_scare_duration[i] ==0:
                        i+=1
                    index=make_scare_index[i]
                    self.effect[index](self.action,self)(gamestate)
                    self.effect_duration[index] -=1
            else:
                if make_scare in self.effect and any(make_scare_duration):
                    ...
                    for i in make_slow_index:
                        index=i
                        if self.effect_duration[i]>0:
                            self.effect_duration[i] -=1
                        else:
                            self.effect_duration[i]=0
                else:
                    make_slow(self.action,self)(gamestate)
                    for i in make_slow_index:
                        index=i
                        if self.effect_duration[i]>0:
                            self.effect_duration[i] -=1
...
def make_scare(action, bee):
    ...
    def new_action(gamestate):
         ...
         if bee.blocked():
             bee.sting(bee.place.ant)
         elif bee.armor > 0 and destination.name != 'Hive':
             bee.move_to(destination)
    return new_action
