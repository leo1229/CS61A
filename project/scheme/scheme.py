...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        helper = lambda env: lambda expr: scheme_eval(expr, env)
        operands = rest.map(helper(env))
...
def eval_all(expressions, env):
    ...
    if expressions == nil:
        return None
    copy = Pair(expressions.first, expressions.rest)
    while copy.rest != nil:
        scheme_eval(copy.first, env)
        copy = copy.rest
    return scheme_eval(copy.first, env)
    # END PROBLEM 7
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        function = do_lambda_form(Pair(target.rest, expressions.rest) ,env)
