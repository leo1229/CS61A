...
def distance(city1, city2):
    ...
    """
    "*** YOUR CODE HERE ***"
    lat_differ=get_lat(city1)-get_lat(city2)
    lon_differ=get_lon(city1)-get_lon(city2)
    dis=sqrt((lat_differ)**2+lon_differ**2)
    return dis
...
def closer_city(lat, lon, city1, city2):
    ...
    """
    "*** YOUR CODE HERE ***"
    goal_city=make_city('goal',lat,lon)
    d1=distance(goal_city,city1)
    d2=distance(goal_city,city2)
    if d1<d2:
...
def nut_finder(t):
    ...
    if is_leaf(t):
        ...
        if label(t)=='nut':
            ...
            result=False
            while i<len(new_t):
                result=result or nut_finder(new_t[i])
                i+=1
    return result
...
def sprout_leaves(t, values):
    ...
    if is_leaf(t):
        ...
        result=[label(t)]
        while i<len(new_t):
            result+=[sprout_leaves(new_t[i], values)]
            i+=1
    return result
